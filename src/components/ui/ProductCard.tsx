...
export default function ProductCard({
    ...
    const [chosenExtra, setChosenExtra] = useState<Ingredient[]>([])
    ...
    return (
        <div onClick={(): void => setActive(false)}
             className={`${active ? "modal active" : "modal"}`}>
            <div onClick={e => e.stopPropagation()}
                className={`${active ? "scale-105" : "scale-100 transition-all duration-300"} fixed top-[50%] left-[50%] -translate-x-1/2  -translate-y-1/2 bg-white rounded-xl w-220 h-152.5
                ...
                {fullMenu.filter((item: Pizza | Drink): boolean => item.id === productId.id && (isPizza(item) ? item.type === productId.type : productId.type === "DRINK"))
                    .map((item: Pizza | Drink): JSX.Element => {
                            ...
                            function countExtras(): number {
                                const result: (Ingredient | undefined)[] = chosenExtra.map((item: Ingredient): Ingredient | undefined =>
                                    ingredients.find((ing: Ingredient): boolean => ing.id === item.id))
                                            ...
                                            <div className="mt-2 flex grid grid-cols-3 gap-x-2 gap-y-2">
                                                {ingredients.map((ingredient: Ingredient): JSX.Element =>
                                                    <div
                                                        ...
                                                        className={`${chosenExtra.some((item: Ingredient): boolean => item.id === ingredient.id) ? "ring-1 ring-red-700" : ""} 
