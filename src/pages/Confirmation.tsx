    ...
    return (
        <div className="h-[100vh]">
            <div className="mx-auto mb-20 w-250 min-h-150 h-auto bg-white rounded-xl shadow-lg py-8 px-16 font-medium">
                <h1 className="text-3xl">{t("orderAccepted")}</h1>
                ...
                <div className="border-y-1 border-neutral-200 py-4 flex flex-col gap-2">
                    <p className="text-xl">{orderDetails?.orderQuantity} {orderDetails?.orderQuantity > 1 ? t("products") : t("product")}, {orderDetails?.orderPrice}₽</p>
                    <p className="text-neutral-500">{t("deliveryAddress")}: {location.state?.userInfo.deliveryAddress}</p>
                </div>
